...
export default function VendorDashboardPage() {
  ...
  const handleGenerateAndSendInvoice = () => {
    ...
    const completedHistoryItem = {
      ...
      rating: 5,
      status: 'Completed',
      review: `Service completed. ${paymentLabel} of ₹${grandTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })} received on-site by vendor.`,
      invoiceData: invoiceDataObj
    };

    setTodayEarnings(prev => prev + grandTotal);
      ...
      <main className="flex-1 w-full max-w-[1380px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-16">
        ...
        {activeTab === 'profile' && (
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4 no-print-bg">
            <h3 className="text-lg font-extrabold text-slate-900 border-b border-slate-100 pb-3">Vendor Credentials</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div><strong className="text-slate-400 uppercase block">Name:</strong> {vendorProfile.name}</div>
              <div><strong className="text-slate-400 uppercase block">Phone:</strong> {vendorProfile.phone}</div>
              <div><strong className="text-slate-400 uppercase block">Vendor UPI ID:</strong> {vendorProfile.upiId}</div>
              <div><strong className="text-slate-400 uppercase block">NABL Certification:</strong> {vendorProfile.nablId}</div>
              <div><strong className="text-slate-400 uppercase block">Bank Account:</strong> {vendorProfile.bankAccount}</div>
            </div>
          </div>
        )}
