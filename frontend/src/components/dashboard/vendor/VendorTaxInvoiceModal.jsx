        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/75 backdrop-blur-xs overflow-y-auto tax-invoice-modal-overlay">
          ...
          >
            ...
            <div className="p-6 sm:p-8 space-y-6 text-slate-800 bg-white" id="invoice-document-body">
              ...
              {/* Travel & Route Verification Badge (if travel KM recorded) */}
              {(generatedInvoiceData.travelDistanceKm > 0 || generatedInvoiceData.mapScreenshot) && (
                <div className="bg-blue-50/70 border border-blue-200/90 rounded-2xl p-3 sm:p-3.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-extrabold text-blue-950">
                          {generatedInvoiceData.travelDistanceKm ? `${generatedInvoiceData.travelDistanceKm} KM Traveled` : 'Travel Distance Logged'}
                        </span>
                      ...
                      <p className="text-[11px] text-blue-800/80">
                        Technician traveled directly to customer location with verified map route.
                      </p>
                    </div>
                  </div>

                  {generatedInvoiceData.mapScreenshot && (
                    <button
                      type="button"
                      onClick={() => setShowProofPreview(true)}
                      className="no-print text-xs font-bold text-blue-700 hover:text-blue-900 bg-white border border-blue-300 px-3 py-1.5 rounded-xl shadow-xs transition-colors flex items-center gap-1.5 cursor-pointer shrink-0"
                    >
          ...
          {/* Map Screenshot Lightbox */}
          {showProofPreview && generatedInvoiceData.mapScreenshot && (
            <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm">
              <div className="bg-white rounded-3xl p-5 max-w-lg w-full shadow-2xl space-y-4">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  <div className="flex items-center gap-2">
                    <Navigation className="w-4 h-4 text-blue-600" />
                    <h4 className="text-sm font-extrabold text-slate-900">Verified Navigation Map Screenshot</h4>
                  </div>
                  <button
                ...
                <div className="flex items-center justify-between text-xs text-slate-600">
                  <span>Distance: <strong>{generatedInvoiceData.travelDistanceKm || '—'} KM</strong></span>
                  <button
                    onClick={() => setShowProofPreview(false)}
                    className="px-4 py-2 bg-slate-900 text-white rounded-xl font-extrabold text-xs"
                  >
                    Close Preview
                  </button>
                </div>
